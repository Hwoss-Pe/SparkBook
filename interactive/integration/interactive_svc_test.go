...
type InteractiveTestSuite struct {
	...
	db     *gorm.DB
	redis  redis.Cmdable
	server *grpc2.InteractiveServiceServer
}
...
func (s *InteractiveTestSuite) TestIncrReadCnt() {
	testCase := []struct {
	...
	}{
		{
			...
			before: func(t *testing.T) {
				...
				err = s.redis.HSet(ctx, "interactive:test:2",
					"read_cnt", 3).Err()
				assert.NoError(t, err)
			},
			after: func(t *testing.T) {
				...
				cnt, err := s.redis.HGet(ctx, "interactive:test:2", "read_cnt").Int()
				assert.NoError(t, err)
				assert.Equal(t, 4, cnt)
				err = s.redis.Del(ctx, "interactive:test:2").Err()
				assert.NoError(t, err)
			},
			biz:      "test",
			bizId:    2,
		...
		},
		{
			...
			after: func(t *testing.T) {
				...
				cnt, err := s.redis.Exists(ctx, "interactive:test:3").Result()
				assert.NoError(t, err)
	...
	for _, tc := range testCase {
		s.T().Run(tc.name, func(t *testing.T) {
			tc.before(t)
			cnt, err := s.server.IncrReadCnt(context.Background(), &intrv1.IncrReadCntRequest{
				Biz:   tc.biz,
				BizId: tc.bizId,
			})
			assert.Equal(t, tc.wantErr, err)
			assert.Equal(t, tc.wantResp, cnt)
			tc.after(t)
		})
	}
}
	...
	testCases := []struct {
	...
	}{
		{
			...
			after: func(t *testing.T) {
				...
				cnt, err := s.redis.HGet(ctx, "interactive:test:2", "like_cnt").Int()
				assert.NoError(t, err)
				assert.Equal(t, 4, cnt)
				err = s.redis.Del(ctx, "interactive:test:2").Err()
				assert.NoError(t, err)
		...
		},
		{
			...
			after: func(t *testing.T) {
				...
				cnt, err := s.redis.Exists(ctx, "interactive:test:2").Result()
				assert.NoError(t, err)
				assert.Equal(t, int64(0), cnt)
			},
			biz:      "test",
	...
	testCases := []struct {
	...
	}{
		{
			...
			after: func(t *testing.T) {
				...
				cnt, err := s.redis.Exists(ctx, "interactive:test:1").Result()
				assert.NoError(t, err)
				assert.Equal(t, int64(0), cnt)
				// 收藏记录
		...
		},
		{
			...
			before: func(t *testing.T) {
				...
				assert.NoError(t, err)
				err = s.redis.HSet(ctx, "interactive:test:3", "collect_cnt", 10).Err()
				assert.NoError(t, err)
			},
			after: func(t *testing.T) {
				ctx, cancel := context.WithTimeout(context.Background(), time.Second)
				...
				cnt, err := s.redis.HGet(ctx, "interactive:test:3", "collect_cnt").Int()
				assert.NoError(t, err)
	...
	testCases := []struct {
		...
		},
		{
			...
			before: func(t *testing.T) {
				...
				assert.NoError(t, err)
				err = s.redis.HSet(ctx, "interactive:test:3",
